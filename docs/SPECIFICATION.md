...
当两个参与方已经通过 CHP 发现了同一个能力时，`opp.negotiate_capability(local, remote)` 负责验证两份 `opp.rcp.v0.1` 能力信封，并对能力 ID、输入模式和输出模式做规范化内容根的精确求交。该 OPP 结果只表示契约兼容；控制平面可以据此规划路由，但不能代替 OPP 进行能力协商，也不能从协商结果获得权限。
