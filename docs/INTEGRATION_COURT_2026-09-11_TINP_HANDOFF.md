...
- Independent curl replay: [OPP_INDEPENDENT_CURL_REPLAY_2026-09-11.json](C:/Users/User/Documents/RCL/_worktrees/opp-production-audit-v01/evidence/OPP_INDEPENDENT_CURL_REPLAY_2026-09-11.json)
...
- The independent curl/libcurl replay produced the same declared projected response root, while its unprojected wire root differed and was not treated as equivalent.
...
`OPP-THIRD-PARTY-001` remains below independent third-party verification: GitHub supplied the observed data, the executable provider is a TaoWind TINP candidate, the curl replay is only an independent runtime differential, and the OPP consumer is an explicit local fixture.
